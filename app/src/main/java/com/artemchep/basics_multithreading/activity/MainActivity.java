package com.artemchep.basics_multithreading.activity;
...
import com.artemchep.basics_multithreading.adapter.MessageAdapter;
...
public class MainActivity extends AppCompatActivity {
    ...
    private static final int SENT_TO_THREAD = 24;
    private static final int FROM_UI_THREAD = 42;

    private List<WithMillis<Message>> listOfMessage = new ArrayList<>();

    private MessageAdapter mAdapter = new MessageAdapter(listOfMessage);

    private HandlerThread bgThread = new HandlerThread("bgThread");
...
//        showWelcomeDialog();
    ...
    public void insert(final WithMillis<Message> message) {
        ...
        mAdapter.notifyItemInserted(listOfMessage.size() - 1);
        final WithMillis<Message> messageWithStartTime = new WithMillis<>(message.value, System.currentTimeMillis());
        bgThreadHandler.obtainMessage(SENT_TO_THREAD, messageWithStartTime).sendToTarget();
    }

    @UiThread
    public void update(final WithMillis<Message> message) {
        for (int i = 0; i < listOfMessage.size(); i++) {
            if (listOfMessage.get(i).value.key.equals(message.value.key)) {
                listOfMessage.set(i, message);
    ...
    private final Handler.Callback backgroundThreadCallback = new Handler.Callback() {
        ...
        public boolean handleMessage(android.os.Message msg) {
            if (msg.what == SENT_TO_THREAD) {
                ...
                mainThreadHandler.obtainMessage(FROM_UI_THREAD, messageNewWithMillis).sendToTarget();
